...
## Why ECS Fargate over lambdas?

TL;DR- cost of operation under high sustained traffic.
...
Nitpick: Supabase could get overwhelmed.. Probably needs some caching.

## Why AWS AgentCore over any online LLM API

Managed compute, auto-scaling for my agent's execution environment. Trying to keep everything on one platform. Gives me "GenAI observability" on cloudwatch.
...
Supabase DB has some storage in free tier
- should be able to store restaurant data

Data Sources:
- OpenStreetMaps
    - Free, might be outdated and doesn't reliably give me menu though.
- Google Maps API
    - usually has menu links,
    - limits on free tier.
    - OKAY I HAVE $300 IN FREE TRIAL CREDITS LET'S GOOO

Due to budget cuts (me not wanting to do the AI menu scraping), I'm going to skip adding the menu text for now, and hope I don't wake up to a $5000 AWS bill.

So my ingestion pipeline is just Google maps -> Supabase.

## TODO

- [ ] Google Maps -> Supabase pipeline
- [ ] AWS CDK to spin infrastructure up
